...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        const existingImageIndex = imagesJsonData.findIndex(
        ...
        console.log("image.json updated successfully.");
    } catch (error) {
        console.log("Error updating image.json:", error);
    }
};
...
async function getImagesJsonData(bucket) {
    try {
        const response = await s3
            .getObject({ Bucket: bucket, Key: "image.json" })
            .promise();
